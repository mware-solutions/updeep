  ...
  return reduce(
    ...
    (acc, value, key) => {
      ...
      if (Array.isArray(value) && Array.isArray(object[key])) {
        ...
        if (isEqual(updatedValue, object[key])) {
...
function updateArray(updates, object) {
  ...
  let changed = Object.keys(object).length !== updatedKeys.length
  const newArray = [...object]

  updatedKeys.forEach(key => {
    let updatedValue = updates[key] // eslint-disable-line no-use-before-define
